...
def _calc_power(player) -> int:
    """Effective combat power with small random variance."""
    base = player["attack_pow"] + (player["defense_pow"] // 2) + player["level"] * 5
    variance = int(base * 0.2)
    return base + random.randint(-variance, variance)


async def _get_effective_stats(player):
    """Return attack/defense with building bonuses applied."""
...
async def attack(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        async with db.execute(
            "SELECT * FROM players WHERE username=? AND is_banned=0",
            (target_username,)
        ) as cur:
    ...
    for w in recent_wars:
        if w["attacker_id"] == attacker["user_id"] and w["defender_id"] == defender["user_id"]:
            if now - w["timestamp"] < ATTACK_COOLDOWN:
                ...
                await update.message.reply_text(
                    f"⏰ Cooldown! Kamu baru menyerang *{target_username}*.\n"
    ...
    if attacker_wins:
        ...
        # Transfer gold
        new_atk_gold = attacker["gold"] + loot_gold
        new_def_gold = max(0, defender["gold"] - loot_gold)
        # Give attacker EXP & maybe level up
        ...
        while new_exp >= exp_needed(new_level):
            ...
            new_level += 1
        await update_player(attacker["user_id"], gold=new_atk_gold, exp=new_exp, level=new_level)
        await update_player(defender["user_id"], gold=new_def_gold)
        result = "attacker_win"
    else:
        # Attacker loses some HP
...
async def battle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    # Placeholder for future battle callbacks
